...
def check_args(file_args):
    """
    Sanity checks out input and prints out usage if input is not a
    positive integer
    """
    try:
        ...
        print(
            "Usage: create_measurements.sh <positive integer number of "
            "records to create>"
        )
        print(
            "        You can use underscore notation for large number of "
            "records."
        )
...
def build_weather_station_name_list():
    """
    Grabs the weather station names from example data provided in repo
    and dedups
    """
    station_names = []
...
def build_test_data(weather_station_names, num_rows_to_create):
    ...
    # Instead of writing line by line, process a batch and write to disk.
    batch_size = 10000
    ...
    try:
        with open("./data/measurements.txt", "w", encoding="utf-8") as file:
            for s in range(0, num_rows_to_create // batch_size):
                ...
                # Using :.1f is quicker than round at scale.
                prepped_deviated_batch = "\n".join(
                    [
                        f"{station};"
                        f"{random.uniform(coldest_temp, hottest_temp):.1f}"
